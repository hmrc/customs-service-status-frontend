...
import uk.gov.hmrc.customsservicestatusfrontend.TestData.{fakePlannedWork, fakeUnplannedOutage, serviceStatuses}
...
class DashboardControllerISpec extends BaseISpec with MockitoSugar {
  ...
  "show" should {
    "return the correct status with OutageData for the Dashboard page" in {
      ...
      stubGet("/customs-service-status/outages/latest?outageType=Unplanned", Json.stringify(Json.toJson(fakeUnplannedOutage)))
